...
public class ReversiModel {
	...
	void step(int[] coords) {
		...
		case Constants.START -> {
			...
			if (this.turnsTaken % 2 == 0 && turnsTaken > 0 && moveResult && nrPlayers == 2) {
				this.setNextTurn();
			} else if (moveResult && nrPlayers == 4) {
				this.setNextTurn();
			}
		...
		for (int i = 0; i < this.boardSize; i++) {
			...
			for (int j = 0; j < this.boardSize; j++) {
				...
				if (j < boardSize - i) {
					...
					diagonalBottomToRightPath = iteratePathAlgorithm(diagonalBotttomToRightCoords,
							diagonalBottomToRightPath);
...
class Board extends TwoDimensionalGrid<Checker> {
	...
	/*
	 * Checks if the coords are within a designated square - primarily used for the starting scenario. Lower:inclusive, Upper: Exclusive f(x,y)
	 * = true <=> lower<=x<upper && lower<=y<upper
	 */
